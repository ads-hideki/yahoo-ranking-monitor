...
| パス | 内容 |
|------|------|
| `screenshots/YYYYMMDD_HHMM_cat{ID}_{カテゴリ名}_{商品コード}_1i.png` | 1位獲得時の証跡スクショ（順位・カテゴリ名・更新日が写る範囲） |
| `data/history.csv` | 実行ごとの自店商品の順位履歴（Excelで開けるUTF-8 BOM） |
| `data/wins.json` | **商品ごとの累計1位獲得回数**（初回日〜最終日、獲得スロット一覧） |
| `data/latest.json` | 直近実行のサマリー（1位獲得一覧） |
| `data/products.json` | 商品→カテゴリの対応表（自動生成・キャッシュ） |

### 「1位獲得回数」の数え方

`wins.json` の `count` は **1位を観測した監視スロット数（朝/夜単位）** です。
Yahooのデイリーランキングは1日約2回（朝・夜）更新されるため、監視も朝夜の2回に合わせています。
同じスロット内で手動再実行しても二重カウントされません（`(商品, 日付, 朝/夜)` で排除）。
朝・夜どちらも1位なら、その日は最大2回としてカウントされます。
...
ランキングページには「リアルタイム / デイリー / 年間」のタブがあり、本システムは
**デイリー**（＝広告で使う「ランキング1位獲得」の定番。1日約2回更新）を監視します。
「リアルタイム」も存在し（より高頻度に更新）、必要なら監視対象に追加できます
（タブがJS切替のため、`fetch_ranking` でリアルタイムのタブをクリックして取得する処理を足す形）。
ご希望あれば「デイリー＋リアルタイム両方を記録」に拡張可能です。
