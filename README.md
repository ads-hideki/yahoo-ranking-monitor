...
毎朝 **JST 09:00** に、その日の1位商品をメールで通知します（Gmail の SMTP から送信）。

- **デイリー**：その日のデイリー1位の商品
- **リアルタイム**：**直近24時間（前日〜当朝）に1度でも1位になった商品をすべて**（`history.csv` から集計）
- 各商品の1位スクショは**本文に画像として埋め込み表示**（添付なし＝軽量。GitHub Pages上の画像URLを参照）
- 送信前に、当日撮影分の画像が Pages で公開されるまで自動で待機（画像切れ防止）
